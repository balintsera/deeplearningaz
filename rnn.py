...
def rnn(epochs=2, units=50):
    ...
    # Adding LSTM layers
    layers = [
        {'layer': LSTM(units=units, return_sequences=True, input_shape=(X_train.shape[1], 1)), 'dropout': Dropout(0.2)},
        {'layer': LSTM(units=units, return_sequences=True), 'dropout': Dropout(0.2)},
        {'layer': LSTM(units=units, return_sequences=True), 'dropout': Dropout(0.2)},
        {'layer': LSTM(units=units, return_sequences=True), 'dropout': Dropout(0.2)},
        {'layer': LSTM(units=units, return_sequences=True), 'dropout': Dropout(0.2)},
        {'layer': LSTM(units=units), 'dropout': Dropout(0.2)},
        {'layer': Dense(units=1)}
        ]
    for layer in layers:
...
regressor = rnn(epochs=8, units=200)
